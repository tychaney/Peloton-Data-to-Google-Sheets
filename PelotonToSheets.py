...
def simplify_df_all_data(cycling_only):
    ...
    workout_data_we_want['Workout Date'] = workout_data_we_want.loc[:,
                                                                    'Workout Timestamp'].str[:10]
    average_df['Workout Date'] = average_df.loc[:,
                                                'Workout Timestamp'].str[:10]
    ...
    workout_data_we_want = workout_data_we_want[['Workout Date',
                                                 'Length (minutes)',
                                                 'Total Output',
                                                 'Distance (mi)',
                                                 'Calories Burned']].apply(pd.to_numeric,
                                                                           errors='ignore')
    # Aggregation
    # Convert to Date Time
    workout_data_we_want['Workout Date'] = pd.to_datetime(
        workout_data_we_want['Workout Date']).dt.date
    ...
    # Fix Percentages
    average_df['Avg. Resistance'] = average_df['Avg. Resistance'].str.rstrip(
        '%').astype('float') / 100.0
    # Remove instances of none and replace with '0'
    ...
    average_df['Workout Date'] = pd.to_datetime(
        average_df['Workout Date']).dt.date
...
def calculations(
    ...
    values = [
        str(today), str(most_recent_workout), int(goal_distance), round(
            total_distance, 2), round(
            distance_from_goal, 2), round(
                percent_of_goal, 4), str(days_so_far_float), str(days_left_in_year_float), round(
                    pace, 2), round(
                        pace_versus_goal, 2), round(
                            avg_length, 2), round(
                                avg_output, 2), round(
                                    avg_distance, 2), round(
                                        avg_calories, 2), round(
                                            miles_current_month, 2), round(
                                                pace_month, 2), round(
                                                    miles_this_week, 2)]
...
def make_sns_plots(username, moaDF_all_time):
    ...
    ax = sns.boxplot(
        data=moaDF_all_time,
        x=moaDF_all_time.index.to_period('M'),
        y='Avg. Heartrate')
    plt.title(f'{username} Average Heartrate per Ride (by Year)')
    ax.set_xticklabels(ax.get_xticklabels(), rotation=30)
    ...
    plt.savefig(
        f'{graph_path}{username}/{username}_Violin_Average_HR_by_month.jpg')
    plt.clf()
    ...
    ax.set_xticklabels(ax.get_xticklabels(), rotation=30)
    plt.savefig(
        f'{graph_path}{username}/{username}_Violin_Total Output_by_month.jpg')
    plt.clf()
    ...
    ax.set_xticklabels(ax.get_xticklabels(), rotation=30)
    plt.savefig(
        f'{graph_path}{username}/{username}_Boxplot_Total Output_by_month.jpg')
    plt.clf()
...
def make_charts(requested_workout_data, username):
    ...
    figOutput_Distance.update_traces(
        marker=dict(
            size=12, line=dict(
                width=3, color='DarkSlateGrey')), selector=dict(
            mode='markers'))
...
def clear_if_first(sheet1, sheet2, sheet3, goal_distance):
    ...
    if today_date.month == first_day_of_year.month and today_date.day == first_day_of_year.day:
        ...
        sheet1.update(
            'A23',
            'Workout Data by Month (All Time [Most Recent First])')
